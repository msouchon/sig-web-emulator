...
impl Tablet {
    ...
    pub fn draw(&mut self, normalized_x: f32, normalized_y: f32, color_rgba: (u8, u8, u8, u8), width: f32) {
        if let Some((last_normalized_x, last_normalized_y)) = self.last_point_normalized {
            ...
            path_builder.move_to(last_normalized_x * self.image.width() as f32, last_normalized_y * self.image.height() as f32);
            path_builder.line_to(normalized_x * self.image.width() as f32, normalized_y * self.image.height() as f32);
