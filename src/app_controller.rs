...
impl AppController {
    pub fn new(app_window: ui::AppWindow, tablet: Arc<RwLock<tablet::Tablet>>) -> Self {
        Self { app_window, tablet }
    }

    pub fn run(self) -> Result<(), slint::PlatformError> {
        self.app_window
            .set_image(self.tablet.read().to_slint_image());
...

        {
            ...
            let tablet = self.tablet.clone();
            self.app_window
                .on_draw(move |normalized_x, normalized_y, color, width| {
                    let tablet = tablet.clone();
                    let mut tablet = tablet.write();
...

            {
                let tablet = self.tablet.clone();
                self.app_window
                    .on_draw_start(move |normalized_x, normalized_y| {
                        let tablet = tablet.clone();
                        let mut tablet = tablet.write();
